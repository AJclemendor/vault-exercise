...
pub async fn run(config: &Config, client: reqwest::Client) -> Result<Vec<User>> {
    ...
    fund_eth(config, client.clone(), deployer.clone(), &users).await?;
    mint_tokens(config, client.clone(), deployer, token_addr, &users).await?;
    approve_vault(config, client, token_addr, vault_addr, &users).await?;
...
async fn fund_eth(
    ...
    let provider = chain::provider(&config.rpc_url, client, deployer);
...
async fn approve_vault(
    ...
    stream::iter(users)
        .map(|user| {
            ...
            async move {
                let provider = chain::provider(&rpc_url, client, signer);
                let token = MockToken::new(token_addr, &provider);
